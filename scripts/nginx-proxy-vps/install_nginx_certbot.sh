...
if [ ! -d "$SSL_CERT_DIR" ]; then
  echo "🔐 Requesting new SSL certificate for: $BASE_DOMAIN..."
  sudo certbot certonly --dns-cloudflare --dns-cloudflare-propagation-seconds 60 --dns-cloudflare-credentials ~/.secrets/certbot/cloudflare.ini -d $BASE_DOMAIN -d *.$BASE_DOMAIN --email $CERTBOT_EMAIL --agree-tos --non-interactive
